...
/**
 * SERDES driver for Keystone SoC
 *
...
static inline void reg_rmw(void __iomem *addr, u32 value, u32 mask)
{
	u32 reg;
...
static inline void finsr(void __iomem *addr, u32 offset, u32 msb, u32 lsb,
			 u32 val)
{
	reg_rmw(addr + offset, val << lsb, GENMASK(msb, lsb));
}

static inline u32 fextr(u32 val, u32 msb, u32 lsb)
{
...
static inline bool is_four_lane(void __iomem *addr)
{
	u32 val;

	val = ((kserdes_readl(addr, MOD_VER_REG) >> 16) & 0xffff);
	return (val == 0x4eb9) || (val == 0x4ebd);
}

static inline bool is_phy_a(void __iomem *addr)
{
	u32 val;
...
static inline void kserdes_assert_reset(struct kserdes_config *sc)
{
	...
	for (lane = -1; lane < sc->lanes; lane = next_enable_lane(sc, lane))
...
static int kserdes_deassert_reset_poll_others(struct kserdes_config *sc)
{
	...
	for (i = -1; i < sc->lanes; i = next_enable_lane(sc, i))
		lanes_not_ok |= BIT(i);

	if (!is_four_lane(sc->regs))
		ofs = 29;
